...
func CreateOverlay(baseImage, overlayFile string, backingFormat BackingFormat) error {
	...
	// Ensure the overlay's parent dir exists so first-run doesn't fail.
	if err := os.MkdirAll(filepath.Dir(overlayFile), 0755); err != nil {
		return fmt.Errorf("cow: create overlay dir: %w", err)
	}
	// Reject commas in paths: they'd inject qemu-img options. (vhost backend
	// already guards image paths; we guard here too for direct CLI users.)
...
func CreateOverlayTyped(backingFile, overlayFile, backingFormat string) error {
	return CreateOverlay(backingFile, overlayFile, BackingFormat(backingFormat))
}
...
func CommitOverlay(overlayFile, destImage string) error {
	if err := validatePath(overlayFile); err != nil {
		return err
	}
	...
	cmd := exec.Command("qemu-img", "convert", "-O", "raw", overlayFile, destImage)
	if out, err := cmd.CombinedOutput(); err != nil {
...
// validatePath rejects empty and comma-bearing paths. Commas are how qemu-img
// and the vhost backend delimit options, so a comma in a filename would be an
