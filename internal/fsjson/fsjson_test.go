...
// TestWrite covers the Write scenarios as table-driven subtests: successful
// writes (with overwrite and no temp leftovers), unwritable targets, and
// encode failures that must leave the previous content intact.
func TestWrite(t *testing.T) {
	t.Run("round trip and no temp leftovers", func(t *testing.T) {
		dir := t.TempDir()
	...
	t.Run("encode failure keeps target intact", func(t *testing.T) {
		dir := t.TempDir()
		path := filepath.Join(dir, "meta.json")
