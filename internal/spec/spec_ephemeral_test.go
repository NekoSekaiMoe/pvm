// spec_ephemeral_test.go — table-driven validation matrix for
// workspace.ephemeral (non-persistent sandboxes): the flag is accepted on
// its own, rejected in combination with the overlay lifecycle knobs it
// makes meaningless, allowed alongside declared volumes, and participates
// in the fingerprint contract.
package spec
...
// TestEphemeralSpecMatrix covers the workspace.ephemeral validation
// scenarios as subtests: decode, compact_on_exit conflict, overlay
// conflict, volumes allowed, and fingerprint discrimination.
func TestEphemeralSpecMatrix(t *testing.T) {
	cases := []struct {
		name string
		run  func(t *testing.T)
	}{
		{"decode accepted", func(t *testing.T) {
		...
		{"compact_on_exit conflict", func(t *testing.T) {
			s := baseValid()
		...
		{"overlay conflict", func(t *testing.T) {
			s := baseValid()
			s.Workspace.Ephemeral = true
		...
		{"volumes allowed", func(t *testing.T) {
			// Declared persistent volumes are user intent: they stay
			// attached and preserved in ephemeral mode, so the
		...
		{"fingerprint distinguishes", func(t *testing.T) {
			a := baseValid()
